...
function kentsGameOfQuestions(){
    ...
    while (qSixGuesses < 4) {
        ...
        var returnArray = validateAnswer('How old am I?',qSixGuesses,4);
        var userAgeGuess = returnArray[0];
        qSixGuesses = returnArray[1];
        if(userAgeGuess){
...
function validateAnswer(question,guesses,maxLimitGuesses){
    while(guesses < maxLimitGuesses)
    {
        var usersGuess = prompt(question);
        if(usersGuess){
            return [usersGuess, guesses];
        }else{
